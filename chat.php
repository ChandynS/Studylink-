...
/* Loads messages */
$sSQL = "SELECT *FROM messages(sender_id = {$_SESSION["user_id"]} AND receiver_id = $otherUserID) OR
(sender_id = $otherUserID AND receiver_id = {$_SESSION["user_id"]}) ORDER BY sent_at ASC";
...
<?php while ($row = mysqli_fetch_assoc($objResult)) { ?>
    ...
    <p>
        <b>
            <?php echo ($row["sender_id"] == $_SESSION["user_id"]) ? "You" : "User"; ?>:
        </b>
